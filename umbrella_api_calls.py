...
class globalV():
    ...
    firstTimeScript_empty_secret_key_tile = ""
...
def LerConfiguracao():
    global org_id,mgmt_api_key,mgmt_api_secret,language,investigate_api_key
    ...
    if mgmt_api_key == "<Umbrella Management API Key>":
        while MgmtAPI == False:  
            ...
            if mgmt_api_key_input is not None and mgmt_api_key_input != "":
                ...
                messagebox.showerror(globalV.firstTimeScript_empty_mgmg_key_title, globalV.firstTimeScript_ask_mgm_secret, parent=root)

    SecretAPI = False
    if mgmt_api_secret == "<Umbrella Management Secret Key>":
        while SecretAPI == False:  
            ...
            if mgmt_api_secret_input is not None and mgmt_api_secret_input != "":
                ...
                messagebox.showerror(globalV.firstTimeScript_empty_secret_key_tile, globalV.firstTimeScript_empty_secret_key, parent=root)
...
def start_progress_thread(event, pg_bar, tipo):
    ...
    pg_bar.start()
    thread.start()
    root.after(100, check_progress_thread)
...
def menu():
    ...
    filemenu6.add_command(label=globalV.menu_configFile, command=openMenuConfigurationFile)
    menubar.add_cascade(label=globalV.menu_configFile_title, menu=filemenu6)
    
    #Menubar Sair
    filemenu7 = Menu(menubar, tearoff=0, background='white', foreground='#000000')
    filemenu7.add_separator()
    filemenu7.add_command(label=globalV.menu_exit, command=on_closingMenu)
    menubar.add_cascade(label=globalV.menu_exit, menu=filemenu7)

    MenuPrincipal.wm_iconbitmap(iconImageFile)
    MenuPrincipal.title('Cisco Umbrella API Calls - {}'.format(versao))
    MenuPrincipal.minsize(825,295)
...
def SetLang(lang, default):
    ...
    for i in frases['frase']:
        ...
        globalV.firstTimeScript_empty_secret_key_tile = i['firstTimeScript_empty_secret_key_tile']
